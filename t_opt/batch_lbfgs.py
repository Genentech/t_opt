...
import logging
from t_opt.opt_util import Status, ConvergenceOpts, DEFAULT_CONVERGENCE_OPTS
from t_opt.coordinates_batch import SameSizeCoordsBatch
from t_opt.abstract_NNP_computer import EnergyAndGradHelperInterface
...
class BatchLBFGS():
    ...
    def __init__(self, lr = 1,
                 ...
                 history_size = 100, line_search_fn:str = None,
                 prune_high_energy_freq = 999999, prune_high_energy_fract = 0.,
        ...
        assert prune_high_energy_fract is None \
               or (prune_high_energy_fract >= 0 and prune_high_energy_fract <= 1) # pylint: disable=R1716
        ...
        self.plot_data: Optional[List[Any]] = [] if plot_name else None
        self.device = device

    @staticmethod
    def _add_grad(coords:torch.tensor, trust:torch.tensor,
    ...
    @staticmethod
    def trust_by_step_size(coords:torch.tensor, update:torch.tensor,
    ...
    @staticmethod
    def _add_grad_wFilter(coords:torch.tensor, trust:torch.tensor,
    ...
    def wolfe_lineSearch(self, n_iter:int, coords:torch.tensor,
        ...
        st.t = BatchLBFGS._add_grad(coords, st.t, st.d)
        ...
        while ls_step < max_ls:
            ...
            if st.std is not None: st.std[lower_e]  = std[lower_e]
            ...
            BatchLBFGS._add_grad_wFilter(coords, st.t, st.d, any_line_search)
        ...
        if n_iter != self.convergence_opts.max_iter:
            ...
            if log.isEnabledFor(logging.DEBUG):
                ...
                log.debug(f'{n_iter} loss: {st.loss[0:5].detach().cpu().numpy()}, coords[0:5,1,0] {coords[0:5,0,0].detach().cpu().numpy()}')
...
#                         log.debug(f'   x12 {coords[0:5,0:2,0].detach().cpu().numpy()}')
            ...
            BatchLBFGS._add_grad_wFilter(coords, st.t-t_new, st.d, bad_steps)
...
#                 st.iters_with_high_linsearch[st.iters_with_high_linsearch >= 4] -= 1
    ...
    def optimize(self, coords_batch: SameSizeCoordsBatch, energy_helper): # pylint: disable=R0912, R0915
...
#                st.d[~is_valid_step] = d_not_valid_steps
            ...
            if n_iter == 1:
                st.t = BatchLBFGS.trust_by_step_size(a_coords, st.d, 0.1)
            else:
                #                 log.debug(lr)
                st.t = st.lr.clone()
            ...
            if self.line_search_fn == "Armijo":
                ...
                st.t = BatchLBFGS._add_grad(a_coords, st.t, st.d)
            ...
            if log.isEnabledFor(logging.DEBUG):
                # pylint: disable=W1203, W1202
                log.debug(f'{n_iter} loss: {st.loss[0:5].detach().cpu().numpy()}, '
                          f'max_grad: {a_flat_grad_square_max.sqrt().detach().cpu().numpy()} '
                          f'a_coords[0:5,1,0] {a_coords[0:5,0,0].detach().cpu().numpy()}')
            ...
            if st.std is not None: min_std[e_decreased_idx] = st.std[a_e_decreased]
    ...
    def plot(self, n_confs):
        ...
        data[:,0] = [i[0] for i in self.plot_data] # pylint: disable=E1133
        data[:,1:] = [i[1].detach().cpu().numpy() for i in self.plot_data] # pylint: disable=E1133
