...
class SameSizeCoordsBatch:
    """ Batch of coordinates for molecules witht he same atom count """

    def __init__(self, allowed_atom_num:Iterable[int], dtype ):
        """
        ...
        self._coords_list:  List[np.ndarray] = []
        self._at_type_list: List[np.ndarray] = []
        self.allowed_atom_num = frozenset(allowed_atom_num)
        self.dtype           = dtype

        self.coords:torch.tensor        # float: nconf,nAtom,3
        self.atom_types:torch.tensor    # int:   nconf,nAtom
        self.uniq_at_types:torch.tensor
        self.at_type_count:List[torch.tensor]
    ...
    def addConformer(self, xyz:np.ndarray, atomic_nums:np.ndarray):
        ...
        if self.n_atom_per_conf == 0:
            ...
            if len(atomic_nums) != self.n_atom_per_conf:
                raise ValueError(f"only molecules with one size are allowed {self.n_atom_per_conf}")
...
class CoordinateModelInterface(nn.Module):
    ...
    def forward(self, same_size_coords_batch:SameSizeCoordsBatch) -> Tuple[torch.tensor,torch.tensor]:
        r""" compute energies, std from SameSizeCoordsBatch

        std may be None if not computed
