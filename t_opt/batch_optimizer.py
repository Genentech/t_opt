# encoding: utf-8
"""
t_opt.optimizer -- Reads sdf files and optimizes them using NNP.
...
from typing import Sequence, Iterable, Type, List, Optional
import numpy as np
import re
import sys
import copy
...
from t_opt.opt_util import OPT_HARM_CONSTRAINT_TAG, ConvergenceOpts,\
    OPT_ENERGY_TAG, OPT_STATUS_TAG, OPT_STEPS, OPT_STD_TAG, OPT_FORCE_TAG,\
    Status, DEFAULT_CONVERGENCE_OPTS, OPT_STD_PER_ATOM_TAG
from t_opt.coordinates_batch import SameSizeCoordsBatch
from t_opt.batch_lbfgs import BatchLBFGS
...
SPACE_TO_CSV_RE = re.compile(r"([0-9.\]]) ([-+0-9\[])")
...
class BatchOptimizer:
    ...
    def __init__(self, nnp_computer:AbstractNNPComputer, molInStream:Iterable[Type[BaseMol]],
                 ...
                 learn_rate: Optional[float] = 0.3,
                 constraint: Optional[str] = None,
                 harm_constr: Optional[Sequence[float]] = None,
                 line_search:Optional[str]  = None, lbfgs_hist_size:Optional[int] = 100,
                 out_grad:Optional[bool] = False,
                 prune_high_energy_freq: Optional[float] = 2, prune_high_energy_fract: Optional[float] = .3,
        ...
        """ Arguments
            ...
            harm_constr: harmonic constraint pulling back to initial coordinates in [kcal/mol/a^2]
        ...
        """
        self.molIn: iterate.PushbackIterator  = iterate.PushbackIterator(molInStream)
        ...
        self._batch_harm_constr: List[float] = []
                    ...
                    if batch_natoms == 0:
                        ...
                        if self.device is not None:
                            if self.device.type == 'cuda':
                                ...
                                log.debug(torch.cuda.memory_summary())

                        # assume memory requirement goes with nAtom^3
                        batch_size = self.nnp_computer.maxConfsPerBatch(batch_natoms)

                        log.info(f"nAt: {batch_natoms}, batchSize: {batch_size}")
    ...
    def optimizeMols(self, mol_batch: Sequence[BaseMol], harm_constr: Sequence[float] ) \
                    -> None:
        """
            ...
            Parameter
            ---------
               mol_batch (potentially large) batch of cdd_chem.mol.BaseMol objects
        ...
        for mol,conf,e,std,g,stat, n_step in zip(mol_batch,coords.detach().cpu().numpy(), energies, stds, grads.detach().cpu().numpy(), status, n_steps):
            ...
            if std:
                mol[OPT_STD_TAG] = f'{std:.1f}'
                mol[OPT_STD_PER_ATOM_TAG] = f'{std/coordsBatch.n_atom_per_conf:.2f}'
